...
class testCases(unittest.TestCase):
    ...
    def test_lon(self):
        test1 = all([round(float(capitals.loc[i, "lon"]), 0)==round(float(truth.loc[i, "lon"]), 0) for i in capitals.index])
        self.assertTrue(test1,
                        msg="Your longitude values don't match truth! Just grab the first value for each city.")
